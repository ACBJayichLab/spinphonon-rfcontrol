...
class Instrument(object):
    ...
    def get_pickleable_parameters(self):
        ...
        for param in self._parameters:
            _p[param] = {}
            for prop in self._parameters[param]:
                if prop not in ['set_func', 'get_func']:
                    _p[param][prop] = copy.deepcopy(self._parameters[param][prop])
        return _p
...
#                logging.debug('Trying to access cached value, but none available')
    ...
    def set(self, name, value=None, fast=False, **kwargs):
        ...
        result = True
        changed = {}
        if type(name) == dict:#types.DictType:
