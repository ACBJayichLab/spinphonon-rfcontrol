...
if hasattr(pyvisa, '__version__'):
    if version.parse(pyvisa.__version__) >= version.parse('1.6'):
        OLD_VISA = False
...
class VisaInstrument(Instrument):
    ...
    def __init__(self, name, address=None, term_chars=None, **kwargs):
        ...
        self.add_parameter('address', type=str)#types.StringType)
    ...
    def clear(self):
        ...
        if not self._ins.is_open():  # Check if the instrument is not open

            self.open()  # Open the instrument if not already open
